...
const { counties, constituencies, wards, modelValue } = defineProps({
	counties: {
		type: Array,
		default: () => []
	},
...
const selectedCounty = ref(modelValue.county);
const selectedConstituency = ref(modelValue.constituency);
const selectedWard = ref(modelValue.ward);
...
const toggleBoundaries = () => {
	const updatedValue = {
		...modelValue,
		boundariesVisible: !modelValue.boundariesVisible
	};
		...
		<div class="flex gap-2 items-center">
			<div class="flex items-center gap-2">
				<div 
					...
					:class="{
						'text-primary drop-shadow-[0_0_8px_rgba(59,130,246,0.8)]': modelValue.boundariesVisible,
						'hover:text-primary/90': !modelValue.boundariesVisible,
						'opacity-50': !modelValue.boundariesVisible
					}"
				...
				<div 
					v-if="modelValue && modelValue.boundariesVisible && modelValue.boundariesCount > 0"
					class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-primary/10 text-primary border border-primary/20 shadow-sm"
				>
					{{ modelValue.boundariesCount }}
				</div>
			</div>
			<div class="text-sm">
